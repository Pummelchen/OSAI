...
osai_status_t network_stack_process_udp_frame(const uint8_t *frame,
  ...
  if (parse_udp(frame, frame_len, &src_port, &dst_port, &payload_len) == 0) {
    ++g_udp_dropped_count;
    ++g_udp_malformed_count;
    return OSAI_ERR_INVALID;
  }
...
osai_status_t network_stack_process_tcp_frame(const uint8_t *frame,
  ...
  const uint8_t *src = (const uint8_t *)&ip->source;
  uint32_t remote_address =
...
static void emit_latency_snapshot(uint64_t *udp50, uint64_t *udp95,
                                 uint64_t *udp99, uint64_t *tcp50,
                                 uint64_t *tcp95, uint64_t *tcp99) {
...
void network_stack_self_test(void) {
  ...
  uint64_t tcp95;
  uint64_t tcp99;
  emit_latency_snapshot(&udp50, &udp95, &udp99, &tcp50, &tcp95, &tcp99);

  klog(
      "network: udp/tcp path self-test passed udp_p50=%lu p95=%lu p99=%lu "
      "tcp_p50=%lu p95=%lu p99=%lu\n",
      udp50, udp95, udp99, tcp50, tcp95, tcp99);
}
