...
void ipv4_build_header(uint8_t *hdr, uint16_t total_length, uint8_t protocol,
  ...
  hdr[0] = OSAI_IPV4_VERSION_IHL; /* version=4, IHL=5 (20 bytes) */
  hdr[1] = 0;                     /* DSCP/ECN */
  ...
  uint16_t cksum = ipv4_checksum(hdr, OSAI_IPV4_HEADER_SIZE);
  put_be16(hdr + 10, cksum);
}

void ipv4_self_test(void) {
  uint8_t hdr[20];
  ipv4_build_header(hdr, 40, OSAI_IPV4_PROTO_UDP, 0x0a00020f, 0x0a000202);
  kassert(hdr[0] == 0x45);
  kassert(hdr[8] == 64);
  kassert(hdr[9] == OSAI_IPV4_PROTO_UDP);
  ...
  uint16_t cksum = ipv4_pseudo_checksum(0x0a00020f, 0x0a000202,
                                         OSAI_IPV4_PROTO_UDP, 4, payload, 4);
