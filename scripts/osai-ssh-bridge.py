...
_OSAI_ARCHIVE_MAGIC = "OSAIARCHIVE\n"
...
def _archive_parse_entries(archive_text):
    if not archive_text.startswith(_OSAI_ARCHIVE_MAGIC):
        return None
    cursor = len(_OSAI_ARCHIVE_MAGIC)
    out = []
...
def _handle_tar_command(args, cwd):
    ...
    if mode == "-tf":
        ...
        if listing is None:
            return 1, f"osai-ssh: tar: cannot access '{archive}': No such file\n"
        return 0, listing
    ...
    if mode == "-xf":
        ...
        if VIRTUAL_FS._exists_dir(destination) is False:
            return 1, f"osai-ssh: tar: cannot access '{destination}': No such file\n"
    ...
    for source_arg in args[2:]:
        ...
        if not _collect_archive_entries(source, source_base, entries):
            return 1, f"osai-ssh: tar: cannot access '{source_arg}': No such file\n"
...
def _handle_cpio_command(args, cwd):
    ...
    if create:
        ...
        while i < len(args):
            ...
            source_base = os.path.basename(source)
            if not _collect_archive_entries(source, source_base, entries):
                return 1, f"osai-ssh: cpio: cannot access '{args[i]}': No such file\n"
            source_count += 1
            i += 1
        if archive is None:
            return 1, "cpio: missing archive\n"
...
def osai_remote_command(command, cwd="/"):
    ...
    if cmd == "help":
        return (
            0,
            "OSAI SSH commands: pwd ls cd mkdir touch cp grep find head tail echo "
    ...
    if cmd == "ls":
        ...
        while i < len(args):
            ...
            if arg.startswith("-") and arg != "-" and end_of_options is False:
                if arg == "-a":
                    ...
                    return 1, f"osai-ssh: ls: invalid option '{arg}'\n"
                i += 1
                continue
            if target != cwd:
        ...
        if listing is None:
            return 1, (
                f"osai-ssh: ls: cannot access '{target}': "
    ...
    if cmd in ("l", "ll", "la"):
        ...
        base = f"ls {alias}"
        if args:
            return osai_remote_command(f"{base} {' '.join(args)}", cwd)
        return osai_remote_command(base, cwd)
    ...
    if cmd == "cd":
        ...
        if not VIRTUAL_FS.stat(target):
            return 1, f"osai-ssh: cd: no such file or directory: {target}\n"
        if VIRTUAL_FS.stat(target)["type"] != "dir":
            return 1, f"osai-ssh: cd: not a directory: {target}\n"
        return 0, target
    ...
    if cmd == "mkdir":
        ...
        if not VIRTUAL_FS.mkdir(target):
            return 1, f"osai-ssh: mkdir: failed for '{target}'\n"
        return 0, ""
    ...
    if cmd == "touch":
        ...
        if not VIRTUAL_FS.touch(target):
            return 1, f"osai-ssh: touch: failed for '{target}'\n"
        return 0, ""
    ...
    if cmd == "cat":
        ...
        if content is None:
            return 1, f"osai-ssh: cat: cannot open '{target}': No such file\n"
        return 0, content
    ...
    if cmd == "write":
        ...
        if not VIRTUAL_FS.write(path, payload):
            return 1, f"osai-ssh: write: failed for '{path}'\n"
        return 0, ""
    ...
    if cmd == "cp":
        ...
        if not VIRTUAL_FS.cp(src, dst):
            return 1, f"osai-ssh: cp: failed to copy '{src}'\n"
        return 0, ""
    ...
    if cmd == "grep":
        ...
        if matches is None:
            return 1, f"osai-ssh: grep: cannot open '{target}'\n"
        return 0, matches
    ...
    if cmd == "find":
        ...
        while i < len(rest):
            ...
            if token == "-name":
                ...
                continue
            if token.startswith("-"):
                return 1, f"osai-ssh: find: unsupported option '{token}'\n"
        ...
        if result is None:
            return 1, f"osai-ssh: find: cannot access '{target}': No such file\n"
        return 0, "\n".join(result) + ("\n" if result else "")
    ...
    if cmd == "head" or cmd == "tail":
        ...
        if output is None:
            return 1, f"osai-ssh: {cmd}: cannot open '{target}'\n"
        return 0, output
    ...
    if cmd == "mv":
        ...
        if not VIRTUAL_FS.mv(src, dst):
            return 1, f"osai-ssh: mv: failed to move '{src}'\n"
        return 0, ""
    ...
    if cmd == "rm":
        ...
        if not VIRTUAL_FS.rm(target, allow_dir=False):
            return 1, f"osai-ssh: rm: failed for '{target}'\n"
        return 0, ""
    ...
    if cmd == "rmdir":
        ...
        if not VIRTUAL_FS.rm(target, allow_dir=True):
            return 1, f"osai-ssh: rmdir: failed for '{target}'\n"
        return 0, ""
    ...
    if cmd == "stat":
        ...
        if info is None:
            return 1, f"osai-ssh: stat: cannot access '{target}': No such file\n"
        return 0, (
            f"path={target}\n"
            f"type={info['type']}\n"
            f"size={info['size']}\n"
...
def run_exec(channel, command):
    status, output = osai_remote_command(command, cwd="/")
...
def run_shell(channel, server):
    def prompt():
        return f"osai:{server.cwd} $ "

    _send_line(channel, "OSAI remote shell")
    ...
    while True:
        ...
        status, output = osai_remote_command(command, cwd=server.cwd)
        ...
        if status != 0 and output == "":
            _write_text(
                channel,
                f"osai-ssh: command '{command}' exited with status {status}\r\n",
            )
