...
int main(void) {
  ...
  if (osai_thread_group_run(6, 256, &threads, &thread_checksum) < 0 ||
      threads != 6 || thread_checksum == 0) {
    osai_log("/bin/smptest: user thread group syscall failed\n");
    return 1;
  }
  (void)osai_osctl("osctl ps");
  osai_log_u64("/bin/smptest: workers=", workers, "\n");
  osai_log_u64("/bin/smptest: checksum=", checksum, "\n");
  osai_log_u64("/bin/smptest: user_threads=", threads, "\n");
  osai_log_u64("/bin/smptest: thread_checksum=", thread_checksum, "\n");
  osai_log("/bin/smptest: app-requested SMP worker set passed\n");
  osai_log("/bin/smptest: POSIX-style arbitrary user thread group passed\n");
